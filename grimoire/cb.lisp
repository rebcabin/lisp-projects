...
(defmethod initialize-instance :after ((o circular-buffer) &rest rest &key size granularity)
  ...
  ;; Pump up to nearest higher multiple of granularity (like ceil size granularity).
  (let ((actual-size (+ (- size (mod size granularity)) granularity)))
    ;; Rearranging: a - s is the overhang or "charge," == g  -  s % g
...
(defun pr-size (o)
  (print (format nil "cb-size:  ~A" (cb-size  o))))

(defun pr-lim (o)
  (print (format nil "cb-limit: ~A" (cb-limit o))))

(defun mini-test ()
  (let ((b (make-instance 'circular-buffer :size 4)))
    (print b)
    ...
    (print (format nil "cb-size:  ~A" (cb-size  b)))
    (do-circular-buffer (i b)
      (print i))
    ...
    (loop for i from 0 to 5
          do (print (format nil "cb[~A]: ~A" i (cb-elt i b))))
