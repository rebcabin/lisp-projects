...
;;; 1) The first component is the Kernel of the Genetic Programming
;;; implementation.  This is the domain-independent code which can be
;;; used to execute a variety of different problems, three of which are
;;; provided.

;;; 2) The second component concerns optimizations for the evaluation of
;;; individuals.  These include the function Fast-Eval, which can be
;;; used in your fitness functions to speed things up.  Also shown are
;;; the "pseudo-macro"s shown in the book.  The instalation of this form
;;; of optimization is not portable between different Common Lisp
;;; implementations.  Source conditionalizations have been provided so
;;; that this code should work without alteration under:
...
;;; If you are running under anything other than these you may be able
;;; to use one or other of the implementations (search for #+), but you
;;; may have to hack something new up for yourself.  The example of the
;;; implementations provided should make life simpler.  It is worth
;;; noting that the performance improvement that can result from the
;;; pseudo-macro mechanism is usually substantial and can sometimes be
;;; tenfold.  Thus, if you are likely to be addressing any problems in
;;; which control of evaluation of arguments is necessary, particularly
;;; in problems that exhibit side-effects as a result of evaluating
;;; either the functions or the terminals in the function/terminal sets
;;; then is is very much worth your while to get this working on your
;;; own particular implementation.

;;; 3) The third component is the domain-independent part of the
;;; program-editing tool.

;;; 4) Rules for simplifying Boolean sexpressions.

;;; 5) Problem-specific code for the symbolic regression problem for
;;; 0.5x**2.

;;; 6) Problem-specific code for the Boolean 3-Majority-On problem

;;; 7) Problem-specific code for the Discrete Non-Hamstrung Squard Car
;;; problem.

;;; 8) Is a pair of test functions.  Test-GPP will execute all of the
;;; example test s-expressions shown in the book, printing out the tests
;;; as they are performed.  Time-Test-GPP runs test-GPP, sending the
;;; output from the tests to a log file, printing out the time taken at
;;; the end.
...
;;; macro and fast-eval optimizations.  If you want to use these you
;;; need only compile this section out of the editor or remove the #|
;;; and |# marks and recompile the file.

;;; Notes: When compiling you might get a warning about there being two
;;; definitions for the variable X and for the functions
;;; sort-population-by-fitness and define-function-set-for-MAJORITY-ON.
;;; This is intentional, so as to make the code below mirror the book as
;;; accurately as possible.  Although these warnings will not in any way
;;; prevent the GPP from working, you may choose to remove the second
;;; definition of (defvar X) and also whichever of the definitions of
;;; sort-population-by-fitness you do not want.  The default version of
;;; sort-population-by-fitness that you will get (i.e. the second) is
;;; the one likely to be most reproducible in behavior across platforms,
;;; but not necessarily the fastest.  The first definition of
;;; define-function-set-for-MAJORITY-ON is the more common usage for
;;; Boolean problems with each function represented in the function set
;;; just once.
...
(defvar *fitness-proportionate-reproduction-fraction* :unbound
  "The fraction of the population that will experience
   fitness-proportionate reproduction (with reselection) during each
   generation")
...
(defun run-genetic-programming-system
  ...
  ;; Get the six problem-specific functions needed to specify this
  ;; problem as returned by a call to problem-function
    ...
    (multiple-value-bind (function-set argument-map)
      ...
      (let* ((terminal-set (funcall terminal-set-creator))
             ...
             (population (create-population
                          ...
                          terminal-set
                          seeded-programs))
             ;; Define fitness cases using fitness-cases-creator
             (fitness-cases (funcall fitness-cases-creator))
             ;; New-Programs is used in the breeding of the new
             ;; population.  Create it here to reduce consing.
...
(defun describe-parameters-for-run
  ...
  (format t "~%Maximum depth of individuals after crossover:~50T~D"
          *max-depth-for-individuals-after-crossover*)
  (format t "~%Fitness-proportionate reproduction fraction:~50T~D"
  ...
  (format t "~%Number of fitness cases:~50T~D" *number-of-fitness-cases*)
...
(defun create-population
  ...
  (let ((population                  (make-array size-of-population))
    ...
    (do ((individual-index 0))
      ...
      (let ((new-program
        ...
        (cond ((< individual-index (length seeded-programs))
               ;; Still have room for a new seeded-program
              ...
              ((not (gethash new-program
               ...
               ;; Haven't seen this program before in the population
               (setf (aref population individual-index)
                     (make-individual :program new-program))
    ...
    ;; Flush out uniquifier table so that no pointers
  ...
  (let ((choice (nth (random-integer (length terminal-set))
    ...
    (case choice
      (:floating-point-random-constant
       ...
       ;; Double precision is [NOT NECESSARILY IN 2014] more expensive
...
(defun create-individual-program
    (function-set
     ...
     top-node-p
     full-p)
  "Creates a program recursively using the specified functions and
   terminals.  Argument map determines how many arguments each function
   in the function set is supposed to have.  Allowable depth is the
   remaining depth of the tree we can create; when we hit zero we only
   select terminals.  Top-node-p is true only when we are being called
   as the top node in the tree.  This allows us to make sure that we
   always put a function at the top of the tree.  Full-p indicates
   whether this individual is to be maximally bushy or not."
               ...
               (let ((function            (nth choice function-set))
                 ...
                 (cons function
                       (create-arguments-for-function
                        ...
                        (- allowable-depth 1)
                        full-p)))
               ;; Otherwise, we chose an atom, so pick it out.
               (choose-from-terminal-set terminal-set))))))
...
(defun evaluate-fitness-of-population
    (population
     fitness-cases
     fitness-function)
  ...
  (dotimes (individual-index (length population))
    (let ((individual (aref population individual-index)))
      (multiple-value-bind
        ...
        (setf (individual-hits individual)
              hits)))))
...
(defun crossover-at-any-points (male female)
  ...
  (let ((male-point
    ...
    (let ((new-male   (list (copy-tree male)))
      ...
      ;; and female-point
      (multiple-value-bind
            (male-subtree-pointer male-fragment)
